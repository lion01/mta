...
/**
 * Travel Component Withdrawitems Model
 *
 * @package		Joomla
 * @subpackage	Travel
 *
 */
class TravelModelWithdrawitems extends TravelModelList
{
	var $_name_sing = 'withdrawitem';
	...
	function _buildQueryWhere($where = array())
	{
		...
		if (isset($this->_active['filter']) && $this->_active['filter'])
		{
			...
			$filter_withdraw_id = $this->getState('filter.withdraw_id');
			if ($filter_withdraw_id != '')		$where[] = "a.withdraw_id = " . (int)$filter_withdraw_id . "";
	...
	function _buildQueryOrderBy($order = array(), $pre_order = 'a.sale_id')
	{
