...
class os_payments {	
	...
	function getPaymentMethods($loadOffline = true, $onlyRecurring = false) {
		...
		if (!$methods) {
			...
			$db = & JFactory::getDBO() ;
			if ($loadOffline) {
				$sql = 'SELECT * FROM #__osmembership_plugins WHERE published=1 ' ;
			} else {
				$sql = 'SELECT * FROM #__osmembership_plugins WHERE published=1 AND name != "os_offline" ' ;
			}			
			if ($onlyRecurring) {
	...
	function loadPaymentMethod($name) {
		$db = & JFactory::getDBO() ;
		$sql = 'SELECT * FROM #__osmembership_plugins WHERE name="'.$name.'"';
		$db->setQuery($sql) ;
	...
	function getDefautPaymentMethod() {
		$db = & JFactory::getDBO() ;
		$sql = 'SELECT name FROM #__osmembership_plugins WHERE published=1 ORDER BY ordering LIMIT 1';
		$db->setQuery($sql) ;		
