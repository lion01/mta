...
/**
 * Travel Component TravelModelItem Model
 *
 * @package		Joomla
 * @subpackage	Travel
 *
 */
class TravelModelItem extends JModelItem
{
	/**
	 * id
	 *
...
//		$itemParams->loadString((isset($item->attribs)?$item->attribs:$item->params));
	...
	function _deleteFiles($cids, $fileFields)
	{
		if (!count($fileFields))
			return;
		...
		//Delete images
		$query = "SELECT `" . implode("`, `", array_keys($fileFields))
		...
		{
			$dir = $config->get('upload_dir_' . $this->_name_plur . '_' . $fieldName, '[COM_SITE]' .DS. 'files' .DS. $this->_name_plur . '_' . $fieldName);
			...
			foreach($files as $fileObj)
			{
				...
				if (!TravelFile::deleteFile($imagePath, $op))
					$errors = true;
	...
	{
		$globalParams = JComponentHelper::getParams('com_travel', true);
		$this->setState('params', $globalParams);



		if (defined('JDEBUG'))
			$_SESSION["Travel"]["Model"][$this->getName()]["State"] = $this->state;
