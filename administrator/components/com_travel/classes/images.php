...
@define("TRAVEL_IMAGES_MAX_WIDTH", 1000);
@define("TRAVEL_IMAGES_MAX_HEIGHT", 1000);
@define("TRAVEL_IMAGES_ALOWED_SIZES", '');
@define("TRAVEL_IMAGES_PHYSICAL_THUMB", true);		//Create an hidden thumb file with parameters
@define("TRAVEL_IMAGES_FALLBACK_NAME", ".notfound.png");
@define("TRAVEL_IMAGES_FALLBACK_ROOT", JPATH_ADMIN_TRAVEL .DS. "images");

require_once(JPATH_ADMIN_TRAVEL .DS. "classes" .DS. "file.php");


class TravelImages  extends TravelFile
{
	...
	function searchFile(&$create)
	{
		...
		$fallback = $this->dir .DS. TRAVEL_IMAGES_FALLBACK_NAME;
		if (JFile::exists($fallback))
		{
			$create = 'fly';
			return $fallback;
		}
		...
		$fallback = TRAVEL_IMAGES_FALLBACK_ROOT .DS. TRAVEL_IMAGES_FALLBACK_NAME;
		if (JFile::exists($fallback))
		{
			$create = 'fly';
			return $fallback;
		...
		switch($create)
		{
			case 'thumb':
				...
				else
				{
					...
					if (($size == "0x0") || in_array($size, explode(",", TRAVEL_IMAGES_ALOWED_SIZES)))
					{
						$image = $this->load();
...
*/
    ...
    function writeThumb($image, $name=null)
    {
    	...
    	if (!TRAVEL_IMAGES_PHYSICAL_THUMB)
	...
	{
		$this->width = min($width, TRAVEL_IMAGES_MAX_WIDTH);
	}

    function height($height='') {
        $this->height = min($height, TRAVEL_IMAGES_MAX_HEIGHT);
    }
