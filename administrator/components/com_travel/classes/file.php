...
@define("TRAVEL_UPLOAD_EXTENSIONS_JOINED", "");

class TravelFile extends JFile
{
	...
	function fileExtension($file)
	{
		...
		foreach(explode(",", TRAVEL_UPLOAD_EXTENSIONS_JOINED) as $joinedExt)
		{
	...
	function downloadUrl($path, $dirPattern, $options = array())
	{
		...
		$url = JURI::base(true) . "/index.php?option=com_travel&task=file"
			.	"&path=" . $path
			. 	"&action=download";
	...
	function returnFile($mode = null)
	{
		...
		)
		{
			...
			require_once(JPATH_ADMIN_TRAVEL .DS. 'classes' .DS. 'images.php');
			$thumb = new TravelImages($filePath, $mime);
		...
		else if (!JFile::exists($filePath))
		{
			...
			$msg = JText::sprintf( "TRAVEL_UPLOAD_FILE_NOT_FOUND", $path);
