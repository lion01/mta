...
class TravelModelAgents extends TravelModelList
{
	...
	function __construct($config = array())
	{
		...
		$this->set('filter_vars', array(
			'expired_date_from' => 'date:%Y-%m-%d',
			'expired_date_to' => 'date:%Y-%m-%d',
			'expired_date' => 'string'
				));
	...
	/**
	 * Method to build a the query string for the Agent
	 *
	 * @access public
	 * @return integer
	 */
	...
	function _buildQuery_default()
	{
		...
		$query = ' SELECT a.*'
			...
			. ' FROM `#__travel_agents` AS a '
					...
					.	' LEFT JOIN `#__users` AS _parent_ ON _parent_.id = a.parent'
	...
	function _buildQueryWhere($where = array())
	{
		...
		// Range : expired_date
			$filter_expired_date_from = $this->getState('filter.expired_date_from');
			if ($filter_expired_date_from != '')		$where[] = "DATEDIFF(a.expired_date, " . $db->Quote($filter_expired_date_from) . ") >= 0";

			$filter_expired_date_to = $this->getState('filter.expired_date_to');
			if ($filter_expired_date_to != '')		$where[] = "DATEDIFF(a.expired_date, " . $db->Quote($filter_expired_date_to) . ") <= 0";
