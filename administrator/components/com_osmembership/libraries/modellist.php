...
class OSModelList extends LegacyModel {
	/**
	 ...
	 * @var string
	 */		
	var $defaultOrdering = null ;
	/**
	 * Title field using for searching
	 * @var string
	 */
	var $titleField = null ;
	/**
	...
	public function __construct($config = array())
	{
		...
		$baseStateVars = array('search' => array('', 'string', 1),//default, type, store in session
	...
	function _buildQuery()
	{
		...
		$query = 'SELECT a.* FROM '.$this->mainTable.' AS a '		
		. $where		
		. $orderby
		;
	...
	function _buildContentWhereArray() {
		...
		if ($state->search) {
			...
			if (is_array($this->titleField)) {
				$whereOr = array() ;
				foreach ($this->titleField as $titleField) {
					$whereOr[] = " LOWER($titleField) LIKE ".$search;
				}
				$where[] = ' ('.implode(' OR ', $whereOr).') ' ;
			} else {
				$where[] = 'LOWER('.$this->titleField.') LIKE '.$db->Quote( '%'.$db->getEscaped( $state->search, true ).'%', false );
			}
		}
